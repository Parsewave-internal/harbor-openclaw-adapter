...
class OpenClaw(BaseInstalledAgent):
    ...
    def _resolve_auth_profiles_b64(self) -> str | None:
        ...
        default = Path.home() / ".openclaw" / "agents" / "main" / "agent" / ("auth-profiles.json")
        if default.is_file():
    ...
    async def setup(self, environment: BaseEnvironment) -> None:
        ...
        if auth_b64:
            self.logger.debug("OpenClaw auth: injecting auth-profiles.json for agent %s", agent_id)
            await self.exec_as_agent(
                environment,
                command=(
    ...
    async def run(
        ...
        reasoning_path = (EnvironmentPaths.agent_dir / "openclaw.reasoning.jsonl").as_posix()
    ...
    def _collect_tool_calls(content: Any) -> list[ToolCall]:
        ...
        for item in content:
            ...
            tool_calls.append(ToolCall(tool_call_id=tc_id, function_name=name, arguments=args))
        return tool_calls
    ...
    def _convert_session_to_trajectory(self, session_file: Path) -> Trajectory | None:
        ...
        for ev in events:
            ...
            if not isinstance(msg, dict):
                continue
            role = msg.get("role")
            timestamp = ev.get("timestamp") if isinstance(ev.get("timestamp"), str) else None
    ...
    def populate_context_post_run(self, context: AgentContext) -> None:
        ...
        session_files = [p for p in sessions_dir.glob("*.jsonl") if p.name != "sessions.json"]
