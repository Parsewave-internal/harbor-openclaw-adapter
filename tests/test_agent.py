...
from harbor_openclaw import OpenClaw


class TestOpenClawBasic:
    """Agent name + public import surface."""

    def test_package_exports_openclaw(self):
        from harbor_openclaw import OpenClaw as ReExported

        assert ReExported is OpenClaw
...
class TestResolveAuthProfilesB64:
    ...
    def test_auth_profiles_path_is_read_and_b64_encoded(self, tmp_path, monkeypatch, temp_dir):
        ...
        assert agent._resolve_auth_profiles_b64() == base64.b64encode(payload).decode("ascii")
    ...
    def test_falls_back_to_default_home_path(self, tmp_path, monkeypatch, temp_dir):
        ...
        default = tmp_path / ".openclaw" / "agents" / "main" / "agent" / "auth-profiles.json"
...
class TestOpenClawConfigInjection:
    ...
    ``tools.exec.applyPatch.workspaceOnly=false`` and
    ``update.checkOnStart=false`` in ``~/.openclaw/openclaw.json``
    ...
    def test_merges_into_existing_config_without_clobbering(self, temp_dir):
        ...
        # Pre-existing siblings inside update.* also survive.
